<h1>BitmaskDropout</h1>

<h2>Description</h2>

<p>BitmaskDropout takes an input floating-point tensor, an optional input ratio (floating-point scalar) and an optional input training_mode (boolean scalar). It produces two tensor outputs: output (floating-point tensor) and mask (optional <code>Tensor&lt;uint32&gt;</code>). If <code>training_mode</code> is true then the output Y will be a random dropout. Note that this Dropout scales the masked input data by the following equation, so to convert the trained model into inference mode, the user can simply not pass <code>training_mode</code> input or set it to false.</p>

<p>output = scale * data * mask,</p>
...
<p align="center"><img alt="node_bitmask_dropout.png" src="/_assets/shared-images/77/778673f0d147-node_bitmask_dropout.png" width="311"/></p>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td valign="top" width="30%"><p align="center"><img alt="input_node_bitmask_dropout" src="assets/input_node_bitmask_dropout.png" width="220"/></p></td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td width="64" valign="top"><img alt="Output_Object_3.Png" src="/_assets/shared-images/e2/e2d605658777-output_object_3.png" width="42"/></td>
      <td valign="top"><strong>mask – T3 : <em>object, </em></strong>the bit-packed output mask.</td>
    </tr>
  </tbody>
</table></td>
    </tr>
...
<p><strong>T1</strong> in (<code>tensor(bfloat16)</code>, <code>tensor(double)</code>, <code>tensor(float)</code>, <code>tensor(float16)</code>) : Constrain input ‘ratio’ types to float tensors.</p>

<p><strong>T2</strong> in (<code>tensor(bool)</code>) : Constrain ‘training_mode’ to boolean tensor.</p>

<p><strong>T3</strong> in (<code>tensor(uint32)</code>) : Constrain output ‘mask’ types to bit-packed uint32 tensor.</p>
