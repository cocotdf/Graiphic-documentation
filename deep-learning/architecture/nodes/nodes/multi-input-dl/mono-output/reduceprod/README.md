...
<p>Computes the product of the input tensor’s elements along the provided axes. The resulting tensor has the same rank as the input if <code>keepdims</code> equals 1. If <code>keepdims</code> equals 0, then the resulting tensor has the reduced dimension pruned. Input tensors of rank zero are valid. Reduction over an empty set of values yields 1. The above behavior is similar to numpy, with the exception that numpy defaults <code>keepdims</code> to <code>False</code> instead of <code>True</code>.</p>

<p align="center"><img alt="node_reduce_prod.png" src="/_assets/shared-images/8e/8ea3756254b6-node_reduce_prod.png" width="299"/></p>
