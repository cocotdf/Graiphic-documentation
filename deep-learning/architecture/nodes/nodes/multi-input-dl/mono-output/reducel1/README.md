...
<p>Computes the L1 norm of the input tensor’s elements along the provided axes. The resulting tensor has the same rank as the input if <code>keepdims</code> equals 1. If <code>keepdims</code> equals 0, then the resulting tensor has the reduced dimension pruned. Input tensors of rank zero are valid. Reduction over an empty set of values yields 0. The above behavior is similar to numpy, with the exception that numpy defaults <code>keepdims</code> to <code>False</code> instead of <code>True</code>.</p>

<p align="center"><img alt="node_reduce_l1.png" src="/_assets/shared-images/40/405d4df11429-node_reduce_l1.png" width="299"/></p>
...
<p><strong>T</strong> in (<code>tensor(bfloat16)</code>, <code>tensor(double)</code>, <code>tensor(float)</code>, <code>tensor(float16)</code>, <code>tensor(int32)</code>, <code>tensor(int64)</code>, <code>tensor(uint32)</code>,<br/>
<code>tensor(uint64)</code>) : Constrain input and output types to numeric tensors.</p>
