...
<p>Setup and add the max pooling 2D layer into the model during the definition graph step. Type : <em><strong>polymorphic</strong><strong>.</strong></em></p>

<p align="center"><img src="/_assets/shared-images/82/8248b8196d5c-max_pool_2d_add_to_graph.png" alt="Max_Pool_2D_Add_To_Graph.Png" width="265" /></p>
...
<h3>MaxPool2D layer</h3>

<p align="center"><img src="assets/1-maxpool2d-layer.png" alt="1 - MaxPool2D layer" width="260" /></p>
...
<p>First, we define the first layer of the graph which is an Input layer (explicit input layer method). This layer is setup as an input array shaped [channels = 7, rows = 5, cols = 3].<br/>Then we add to the graph the MaxPool2D layer.</p>

<p>3 – Run graph</p>

<p>We call the forward method and retrieve the result with the “Prediction 4D” method.<br/>This method returns two variables, the first one is the layer information (cluster composed of the layer name, the graph index and the shape of the output layer) and the second one is the prediction with a shape of [batch_size, channels, pooled_rows, pooled_cols].</p>

<h3>MaxPool2D layer, batch and dimension</h3>

<p align="center"><img src="assets/2-maxpool2d-batch-and-dimension.png" alt="2 - MaxPool2D batch and dimension" width="260" /></p>
